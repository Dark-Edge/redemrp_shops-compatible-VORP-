...
function OpenShopMenu(zone)
    items_list = {}
    local _zone = zone
    for i,v in ipairs (Config.ShopItemsData) do
        local item = Config.ShopItemsData
        table.insert(items_list, {label = item[i].label, value = i , desc = "Item Price: "..item[i].price, price = item[i].price , obj =  item[i].object , name = item[i].item , zone = _zone })
		--print(i,item[i].price,item[i].object,item[i].item,_zone) debug
	end
    StartCam()
    ...
    MenuData.Open(
        ...
        function(data, menu)
			print (data.current.name, data.current.zone, data.current.price)
            TriggerServerEvent('redemrp_shops:BuyItem', data.current.name, 1 , data.current.zone, data.current.price)
        end,
